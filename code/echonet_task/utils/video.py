...
def runPDA(
    data_dir=None,
    output=None,
    file_list = "/zfs/wficai/Data/echonet_data/pda_like_echonet.csv",
    task="Label",
    ...
    # Set default output directory
    if output is None:
        output = os.path.join("output", "video", "PDA_{}_{}_{}_{}".format(model_name, frames, period, "pretrained" if pretrained else "random"))
    os.makedirs(output, exist_ok=True)
    ...
    # Compute mean and std
    mean, std = echonet.utils.get_mean_and_std(PDALikeEcho(
                                                            root=data_dir,
                                                            split="train",
                                                            file_list=file_list)
                                               )
    ...
    # Set up datasets and dataloaders
    dataset = {}
    dataset["train"] = PDALikeEcho(root=data_dir, split="train", **kwargs, pad=12, file_list = file_list)
    if num_train_patients is not None and len(dataset["train"]) > num_train_patients:
        # Subsample patients (used for ablation experiment)
        indices = np.random.choice(len(dataset["train"]), num_train_patients, replace=False)
        dataset["train"] = torch.utils.data.Subset(dataset["train"], indices)
    dataset["val"] = PDALikeEcho(root=data_dir, split="val", file_list = file_list, **kwargs)
