...
#             self.outcome = [f for (f, k) in zip(self.outcome, keep) if k]
    ...
    def __getitem__(self, index):
        ...
        for t in self.target_type:
            ...
            if t == "Label":
                """
                    I think we could update label generation to be faster for training
                """
                # Return 1 if PDA
                if self.labels[index] == "pda":
                    label = np.float32(1)
                # Return 0 if Not Pda
                elif self.labels[index] == "nopda":
                    label = np.float32(0)
                else:
                    raise Exception(f"no label for file {key}") 
                target.append(label)
